...
<?php foreach ($rows as $r) { ?><tr style="border-bottom:1px solid var(--light-gray);"><td style="padding:var(--spacing-md);"><?php echo (int)$r['id']; ?></td><td style="padding:var(--spacing-md);text-align:left;"><?php echo e($r['name']); ?></td><td style="padding:var(--spacing-md);text-align:center;">LKR <?php echo e(number_format((float)$r['price'],2)); ?></td><td style="padding:var(--spacing-md);text-align:center;"><a href="service-form.php?id=<?php echo (int)$r['id']; ?>" class="btn-outline btn-small" style="margin-right:4px;">Edit</a><form method="post" action="service-handler.php" style="display:inline;" onsubmit="return confirm('Delete this service?');"><input type="hidden" name="action" value="delete"><input type="hidden" name="id" value="<?php echo (int)$r['id']; ?>"><button type="submit" class="btn-outline btn-small" style="color:#d32f2f;">Delete</button></form></td></tr><?php } ?>
</tbody></table></div></div>
